...
function pmpro_courses_admin_notice() {
	...
	if ( get_transient( 'pmpro-bp-admin-notice' ) ) { ?>
		<div class="updated notice is-dismissible">
			<p><?php printf( __( 'Thank you for activating. <a href="%s">Click here to add your first courses</a>.', 'pmpro-courses' ), get_admin_url( null, 'edit.php?post_type=pmpro_course' ) ); ?></p>
		</div>
		<?php
...
function pmpro_courses_plugin_action_links( $links ) {
	if ( current_user_can( 'manage_options' ) ) {
		$new_links = array(
			'<a href="' . get_admin_url( null, 'admin.php?page=pmpro_course' ) . '">' . __( 'Settings', 'pmpro-courses' ) . '</a>',
		);
	}
...
function pmpro_courses_plugin_row_meta( $links, $file ) {
	if ( strpos( $file, 'pmpro-courses.php' ) !== false ) {
		$new_links = array(
			'<a href="' . esc_url( 'https://www.paidmembershipspro.com/add-ons/courses/' ) . '" title="' . esc_attr( __( 'View Documentation', 'pmpro' ) ) . '">' . __( 'Docs', 'pmpro-courses' ) . '</a>',
